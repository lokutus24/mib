...
class MibEnqueue extends MibBaseController
{
	...
	public function registerFunction()
	{
		...
		add_action('wp_ajax_set_discount_price_values', array($this, 'set_discount_price_values'));
		add_action('wp_ajax_nopriv_set_discount_price_values', array($this, 'set_discount_price_values'));
	...
	public function set_discount_price_values()
	{
		return $this->set_stairway_values();
	}
	...
	private function getDefaultOtthonStartFilterParams(array $params): array
	{
		...
		if (
			!isset($defaults['price_slider_min_value']) && !empty($this->filterOptionDatas['mib-filter-price_range'])
		...
		if (
			!isset($defaults['slider_min_value']) && !empty($this->filterOptionDatas['mib-filter-square-meter'])
		...
		if (
			!isset($defaults['floor_slider_min_value']) && !empty($this->filterOptionDatas['mib-filter-floor'])
		...
		if (
			!isset($defaults['room_slider_min_value']) && !empty($this->filterOptionDatas['mib-filter-room'])
	...
	private function getArgumentumsByCatalog($params)
	{
		...
		if (isset($params['discountPrice']) && $params['discountPrice'] != 0) {
			$discountPrice = sanitize_text_field($params['discountPrice']);
			if ($discountPrice === '1') {
				$args = array_merge($args, ['discountPrice' => 1]);
			}
		}
